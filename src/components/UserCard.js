...
import { faHeart, faEye, faCheckCircle } from '@fortawesome/free-solid-svg-icons'
import { db } from '../helpers/firebaseConfig'
import { doc, updateDoc } from 'firebase/firestore'

export default function UserCard({ user }) {
    const [masInfo, setMasInfo] = useState(false)
    const [verificado, setVerificado] = useState(user.verified ? true : false)

    const handleVerifyUser = async (event) => {
        event.preventDefault();
        // actualizamos el documento del usuario con el id que nos llega
        // y le agregamos un campo de verificado con valor true
        if (!user.id) return
        if (!user) return
        if (!verificado) {
            ...
            {masInfo && (<>
                ...
                <button
                    className={!verificado ?
                        'text-white rounded-lg p-2 px-4 m-2 shadow-sm bg-blue-500 hover:bg-blue-700'
                        :
                        'text-white rounded-lg p-2 px-4 m-2 shadow-sm bg-red-500 hover:bg-red-700'
                    }
                    onClick={handleVerifyUser}
                >
                    <FontAwesomeIcon icon={faCheckCircle} className='mr-2' color='white' />
                    {!verificado ? "Verificar Usuario" : "Desverificar Usuario"}
                </button>
            </>
            )}
        </div>
